...
bool RG::Homework2::assignment2()
{
    ...
    return (typeid(ci) == typeid(const int)) && (ci == 5)
        && (typeid(cxpi) == typeid(const int)) && (cxpi == 4)
        && (typeid(cip) == typeid(const int*)) && (*cip == ci)
        && (typeid(ipc) == typeid(int* const)) && (*ipc == i)
        && (typeid(cipc) == typeid(const int* const)) && (*cipc == ci);
}
