...
bool RG::Homework2::assignment1()
{
    ...
    // Loo muutuja rp mis on raw pointer tüüpi ja algväärtustatud nullptr-iga
    int* rp = nullptr;

    // Kontrolli, et muutujad on õiget tüüpi ja väärtusega
    return (typeid(ilr1) == typeid(int&)) && (ilr1 == 5)
        && (typeid(irr) == typeid(int&&)) && (irr == 5)
        && (typeid(ilr2) == typeid(int&)) && (ilr2 == 5)
        && (typeid(ip) == typeid(int*)) && (*ip == 5)
        && (typeid(ipp) == typeid(int**)) && (**ipp == 5)
        && (typeid(i2) == typeid(int)) && (i2 == 5)
        && (typeid(up) == typeid(std::unique_ptr<int>)) && (*up == 4)
        && (typeid(sp) == typeid(std::shared_ptr<int>)) && (*sp == 3)
        && (typeid(wp) == typeid(std::weak_ptr<int>)) && (!wp.expired() && *wp.lock() == 3)
        && (typeid(rp) == typeid(int*)) && (rp == nullptr);
}
